...
declare
  ...
  v_submitter_actor text := null;
  v_schedule_audit_actor text := null;
begin
  ...
  if v_leave_row.source = 'agency_schedule' then
    select nullif(btrim(coalesce(audit_row.actor, '')), '')
    into v_submitter_actor
    from public.ob_audit_logs as audit_row
    where audit_row.action = 'agency_leave_request_create'
      and nullif(btrim(coalesce(audit_row.staff_id, '')), '') = nullif(btrim(coalesce(v_leave_row.matched_staff_id, '')), '')
      and coalesce(audit_row.payload ->> 'work_date', '') = v_leave_row.leave_date::text
    order by coalesce(audit_row.created_at, now()) desc, audit_row.id desc
    limit 1;
  end if;

  v_schedule_audit_actor := coalesce(v_submitter_actor, v_actor);
