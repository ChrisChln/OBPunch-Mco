...
set termination_reason = (
  select nullif(btrim(request_row.reason), '')
  from public.ob_employee_termination_requests as request_row
  where request_row.staff_id = employee.staff_id
    and request_row.status = 'approved'
    and nullif(btrim(coalesce(request_row.reason, '')), '') is not null
  order by request_row.reviewed_at desc nulls last, request_row.created_at desc
  limit 1
)
where employee.terminated_at is not null
  and nullif(btrim(coalesce(employee.termination_reason, '')), '') is null
  and exists (
    select 1
    from public.ob_employee_termination_requests as request_row
    where request_row.staff_id = employee.staff_id
...
begin
  ...
  if v_action = 'approve' then
    ...
    set
      ...
      termination_reason = nullif(btrim(v_request.reason), '')
    where staff_id = v_request.staff_id;
