...
begin
  ...
  from (
    select distinct punch.staff_id as old_staff_id
    from public.ob_punches as punch
    where punch.staff_id like 'TEMP%'
      and not exists (
        select 1
        from public.ob_employees as employee
        where employee.staff_id = punch.staff_id
      )
      and not exists (
        select 1
        from public.ob_temp_account_assignments as assignment
        where assignment.source_temp_staff_id = punch.staff_id
      )
  ) as source;
