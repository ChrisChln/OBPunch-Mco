...
exports.runAttendanceAutoCheckout = exports.getMostRecentCutoff = exports.zonedDateTimeToUtc = void 0;
...
const getMostRecentCutoff = (now, timezone = DEFAULT_TIMEZONE, cutoffHour = DEFAULT_CUTOFF_HOUR) => {
    const local = getZonedParts(now, timezone);
    const todayCutoff = (0, exports.zonedDateTimeToUtc)({ year: local.year, month: local.month, day: local.day, hour: cutoffHour, minute: 0, second: 0 }, timezone);
    if (now.getTime() >= todayCutoff.getTime())
        return todayCutoff;
    const previous = addLocalDays(local, -1);
    return (0, exports.zonedDateTimeToUtc)({ ...previous, hour: cutoffHour, minute: 0, second: 0 }, timezone);
};
exports.getMostRecentCutoff = getMostRecentCutoff;
const fetchAllPunchesInWindow = async (supabase, windowStart, cutoffAt) => {
    const rows = [];
    let from = 0;
    while (true) {
...
const fetchEmployeesByStaffId = async (supabase, staffIds) => {
...
const insertAutoCheckout = async (supabase, staffId, cutoffAt) => {
    const row = {
        staff_id: staffId,
        action: 'OUT',
        created_at: cutoffAt,
...
const runAttendanceAutoCheckout = async (supabase, options = {}) => {
    ...
    const cutoff = (0, exports.getMostRecentCutoff)(options.now ?? new Date(), timezone, cutoffHour);
    ...
    const existingCutoffOut = new Set();
    for (const row of punches) {
        ...
        if (action === 'OUT' && createdAt === cutoffAt)
            existingCutoffOut.add(staff);
        if (!latestByStaff.has(staff))
            latestByStaff.set(staff, { action, createdAt });
    }
    ...
    for (const staffId of candidates) {
        ...
        if (!employee || existingCutoffOut.has(staffId) || (0, agencyRules_js_1.isScheduleOnlyAgency)(agency) || (0, employeeStatus_js_1.isEmployeeTerminated)({ terminatedAt: employee.terminated_at })) {
...
exports.runAttendanceAutoCheckout = runAttendanceAutoCheckout;
