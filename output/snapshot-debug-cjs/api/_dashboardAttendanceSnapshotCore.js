...
exports.runDashboardAttendanceSnapshot = exports.buildDashboardAttendanceSnapshotStats = exports.getDashboardSnapshotWorkDate = void 0;
const agencyRules_js_1 = require("../src/shared/agencyRules.js");
const employeeStatus_js_1 = require("../src/shared/employeeStatus.js");
const operationalPunches_js_1 = require("../src/shared/operationalPunches.js");
const positions_js_1 = require("../src/shared/positions.js");
const staffId_js_1 = require("../src/lib/staffId.js");
const _attendanceAutoCheckoutCore_js_1 = require("./_attendanceAutoCheckoutCore.js");
...
const getOperationalDate = (now, timezone, cutoffHour) => {
    ...
    const todayCutoff = (0, _attendanceAutoCheckoutCore_js_1.zonedDateTimeToUtc)({ year, month, day, hour: cutoffHour, minute: 0, second: 0 }, timezone);
...
const getOperationalRange = (workDate, timezone, cutoffHour) => {
    ...
    const start = (0, _attendanceAutoCheckoutCore_js_1.zonedDateTimeToUtc)({ year, month, day, hour: cutoffHour, minute: 0, second: 0 }, timezone);
    const nextLocal = addDays(new Date(`${workDate}T00:00:00`), 1);
    const end = (0, _attendanceAutoCheckoutCore_js_1.zonedDateTimeToUtc)({
...
const getDashboardSnapshotWorkDate = (options = {}) => {
    ...
    const operationalDate = getOperationalDate(options.now ?? new Date(), timezone, cutoffHour);
    if (options.mode === 'actual')
        return addDaysDateOnly(operationalDate, -1);
    return operationalDate;
};
exports.getDashboardSnapshotWorkDate = getDashboardSnapshotWorkDate;
...
const pickLatestByStaff = (rows) => {
    ...
    for (const row of rows) {
        const staff = (0, staffId_js_1.normalizeStaffId)(String(row.staff_id ?? '').trim());
        if (!staff)
            continue;
...
const isEmployeeActive = (employee) => {
    if (!employee)
        return false;
    if ((0, employeeStatus_js_1.isEmployeeTerminated)({ terminatedAt: employee.terminated_at }))
        return false;
...
const resolvePositionName = (value, positionNames) => {
    const normalized = (0, positions_js_1.normalizePositionName)(value);
    if (!normalized)
        return '';
    const exact = positionNames.find((position) => (0, positions_js_1.normalizePositionName)(position).toLowerCase() === normalized.toLowerCase());
    if (exact)
        return (0, positions_js_1.normalizePositionName)(exact);
    return normalized;
};
...
const fetchPositionContext = async (supabase) => {
    ...
    const names = (0, positions_js_1.buildAttendanceTrackedPositionNames)(records);
    const positionNames = names.length ? names : [...positions_js_1.DEFAULT_POSITION_NAMES];
    const departments = new Map();
    const hidden = new Set();
    for (const record of records) {
        const name = (0, positions_js_1.normalizePositionName)(record.name);
        if (!name)
            continue;
        const department = (0, positions_js_1.normalizePositionDepartment)(record.department);
...
const fetchEmployees = async (supabase, staffIds) => {
    ...
    for (let index = 0; index < staffIds.length; index += 200) {
        ...
        const rows = await fetchAllRows((from, to) => supabase
            ...
            .range(from, to));
        for (const row of rows) {
            const staffId = (0, staffId_js_1.normalizeStaffId)(String(row.staff_id ?? '').trim());
...
const fetchPunches = async (supabase, rangeStart, rangeEnd, cutoffHour) => {
    ...
    for (const row of rows) {
        ...
        if (!staffId)
            continue;
        if ((0, operationalPunches_js_1.isExactOperationalCutoffOut)(String(row.created_at ?? ''), row.action, cutoffHour))
            continue;
...
const buildDashboardAttendanceSnapshotStats = async (supabase, options = {}) => {
    const timezone = options.timezone || DEFAULT_TIMEZONE;
    const cutoffHour = clampHour(options.cutoffHour, DEFAULT_CUTOFF_HOUR);
    const workDate = (0, exports.getDashboardSnapshotWorkDate)({ ...options, timezone, cutoffHour });
    ...
    const attendanceTrackedStaffIds = new Set(visibleStaffIds.filter((staffId) => {
        ...
        return !(0, agencyRules_js_1.isScheduleOnlyAgency)(agency) && !isNewHirePlaceholderStaffId(staffId);
    }));
    ...
    for (const key of keys) {
        ...
        stats.push({
            ...
            shift,
            position,
            department: departments.get(position) ?? (0, positions_js_1.normalizePositionDepartment)(null),
...
exports.buildDashboardAttendanceSnapshotStats = buildDashboardAttendanceSnapshotStats;
const runDashboardAttendanceSnapshot = async (supabase, options = {}) => {
    const mode = options.mode === 'actual' ? 'actual' : 'expected';
    const built = await (0, exports.buildDashboardAttendanceSnapshotStats)(supabase, { ...options, mode });
...
exports.runDashboardAttendanceSnapshot = runDashboardAttendanceSnapshot;
