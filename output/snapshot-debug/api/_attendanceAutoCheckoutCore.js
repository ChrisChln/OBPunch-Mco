...
export const zonedDateTimeToUtc = (parts, timezone) => {
...
export const getMostRecentCutoff = (now, timezone = DEFAULT_TIMEZONE, cutoffHour = DEFAULT_CUTOFF_HOUR) => {
    const local = getZonedParts(now, timezone);
    const todayCutoff = zonedDateTimeToUtc({ year: local.year, month: local.month, day: local.day, hour: cutoffHour, minute: 0, second: 0 }, timezone);
...
export const runAttendanceAutoCheckout = async (supabase, options = {}) => {
