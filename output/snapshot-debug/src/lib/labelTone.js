...
export const LABEL_TONE_CLASS_BY_KEY = {
...
export const loadLabelToneMap = () => {
    try {
        ...
        for (const [k, v] of Object.entries(parsed ?? {})) {
            ...
            if (!key || !LABEL_TONE_KEYS.includes(tone))
                continue;
...
export const normalizeLabelToneMap = (value) => {
    const out = {};
...
export const readLabelToneMapFromRows = (rows) => {
    const out = {};
    for (const row of rows ?? []) {
        const label = String(row.label ?? '').trim();
        const tone = String(row.tone ?? '').trim();
        if (!label || !LABEL_TONE_KEYS.includes(tone))
            continue;
        out[label.toLowerCase()] = tone;
    }
    return out;
};
export const buildLabelToneRows = (map, meta) => {
    const rows = [];
...
export const saveLabelToneMap = (map) => {
    try {
...
export const getLabelToneClass = (label, map) => {
